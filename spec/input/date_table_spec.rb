  ...
  describe '.each_row' do
    context 'given start date and end date build consecutive rows' do
      it 'Build two consecutive rows, ensure fiscal year correct' do
        start_date = Date.new(2017, 10, 0o1)
        end_date = Date.new(2018, 12, 0o1)
  ...
  describe '.new' do
    context 'given date build date row with info' do
      it 'Weekday with Quarter starting in Jan' do
        ...
        expect(v.week_start_month_day).to eq '11/19'
        expect(v.week_end_month_day).to eq '11/25'
        expect(v.work_week_start_month_day).to eq '11/20'
        expect(v.work_week_end_month_day).to eq '11/24'
        ...
        expect(v.year_month_int).to eq 201_711
      ...
      it 'Weekend with Quarter starting in Dec' do
        ...
        expect(v.week_start_month_day).to eq '11/26'
        expect(v.week_end_month_day).to eq '12/02'
        expect(v.work_week_start_month_day).to eq '11/27'
        expect(v.work_week_end_month_day).to eq '12/01'
        expect(v.week_number).to eq 47
        expect(v.month_number).to eq 11
        expect(v.month_name).to eq 'November'
        expect(v.quarter).to eq 4
      ...
      it 'Can use get accessor for day' do
        ...
        expect(h['week_start_month_day']).to eq '11/26'
        expect(h['week_end_month_day']).to eq '12/02'
        expect(h['work_week_start_month_day']).to eq '11/27'
        expect(h['work_week_end_month_day']).to eq '12/01'
        ...
        expect(h['year_month_int']).to eq 201_711
