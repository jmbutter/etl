...
module RedshiftHelpers
  def create_table(conn_params, aws_params, table_name, &_block)
...
RSpec.describe 'redshift_batch_factory' do
  context 'generate' do
    let(:table_name) { 'test_table' }
    let(:conn_params) { ETL.config.redshift[:test] }
    let(:aws_params) { ETL.config.aws[:test] }

    it 'Start Time from generated batch equal to value in db' do
      create_table(conn_params, aws_params, table_name) do
        bf = ::ETL::Redshift::BatchFactory.new("SELECT date from #{table_name} LIMIT 1", 3, conn_params, aws_params)
        b = bf.generate
        expect(b.start_time.to_s).to eq('2010-12-15')
      end
    end
    it 'Start Time from generated batch equal days now - backfill days' do
      create_table(conn_params, aws_params, table_name) do
        backfill_days = 3
        bf = ::ETL::Redshift::BatchFactory.new("SELECT date from #{table_name} WHERE id != 1 LIMIT 1", backfill_days, conn_params, aws_params)
        b = bf.generate
        expected_start_time = Time.now.getutc - 60 * 60 * 24 * backfill_days
        expect(b.start_time.strftime("%Y%m%d")).to eq(expected_start_time.strftime("%Y%m%d"))
      end
    end
  end
end
